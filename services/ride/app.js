...
async function publishEvent(routingKey, payload) {
  try {
    ...
    logger.info(
      { event: "publish_failed", routingKey, error: err.message },
...
function getTripStatus(trip) {
  return trip.trip_status || "REQUESTED";
}
...
v1Router.post("/trips", async (req, res) => {
  try {
    const {
      rider_id,
      pickup_location,
    ...
    if (
      ...
      return res.status(400).send({
        error:
          "rider_id, pickup_location, drop_location, city, and distance_km are required"
      });
    }

    const surgeOptions = [1.0, 1.2, 1.5];
    const randomSurge =
      surgeOptions[Math.floor(Math.random() * surgeOptions.length)];
...
v1Router.post("/trips/:id/accept", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${DRIVER_SERVICE_URL}/v1/drivers/${driver_id}`,
      {
        headers: {
...
v1Router.post("/trips/:id/complete", async (req, res) => {
  ...
  try {
    ...
    if (currentStatus === "ACCEPTED") {
      ...
      return res.status(400).send({
        error: `Invalid state transition from ${currentStatus} to COMPLETED`
      });
    }

    const fare = calculateFare(
      trip.distance_km || 0,
      trip.surge_multiplier || 1.0,
      20
    );
    ...
    // Important: mark trip as COMPLETED before calling Payment service
    ...
    trip.payment_status = paymentResponse.data.status || "SUCCESS";
    await trip.save();

    await axios
      .post(
        `${NOTIFICATION_SERVICE_URL}/v1/notifications`,
        {
          trip_id: trip.id,
          rider_id: trip.rider_id,
...
v1Router.post("/trips/:id/cancel", async (req, res) => {
  try {
    ...
    if (!["REQUESTED", "ACCEPTED", "ONGOING"].includes(currentStatus)) {
      return res.status(400).send({
        error: `Invalid state transition from ${currentStatus} to CANCELLED`
      });
    }
    ...
    const cancellationFee = ["ACCEPTED", "ONGOING"].includes(currentStatus)
      ? CANCELLATION_FEE
      : 0;
...
app.get("/metrics", async (req, res) => {
  ...
  completedTripsInDb.set(completedTrips);
